...
function FakeNews() {
    ...
    const handleButtonClick = () => { 
        if(containerRef)
        {
            containerRef.current.style.backgroundColor = "orange";
        }
        if(titleRef)
        {
            titleRef.current.textContent = "[속보] 밥준서, 상록원이 아닌 기숙사 식당에서 발견돼";
        }
        if(bodyRef)
        {
            bodyRef.current.textContent = "상록원 메뉴에 질려 서식지를 바꿨다고 전해져"
        }
    }
